...
export class ChatStateService {
  ...
  private persist(){ sessionStorage.setItem(STORAGE_KEY, JSON.stringify(this.state())); }
  private restore(){
    const raw = sessionStorage.getItem(STORAGE_KEY);
    if (raw) {
      try { this.state.set(JSON.parse(raw)); } catch {}
    }
  }

  reset(){ this.state.set({ question: '', history: [] }); this.persist(); }
